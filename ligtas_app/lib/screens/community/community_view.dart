...
class _CommunityFeedState extends State<_CommunityFeed> {
  ...
  Future<void> _fetchReports() async {
    try {
      ...
      final posts = rawReports.map((r) {
        ...
        final typeLabel = reportType == 'crime' || reportType == 'harassment' ? 'alert'
                        : reportType == 'safe_spot' ? 'tip'
                        : reportType == 'flooding'  ? 'warning'
                        : 'report';
        final lat = (r['lat'] as num?)?.toDouble() ?? 0.0;
        final lon = (r['lon'] as num?)?.toDouble() ?? 0.0;
        return _Post(
          id: (r['id'] ?? 0) as int,
          author: r['username']?.toString() ?? 'Anonymous',
          reputation: 'Candle',
          content: r['description']?.toString().isNotEmpty == true
              ? r['description'].toString()
              : (r['label'] ?? reportType).toString(),
  ...
  String _formatTime(dynamic timestamp) {
    ...
    try {
      ...
      // Backend returns "YYYY-MM-DD HH:MM:SS PHT" or "YYYY-MM-DD HH:MM PHT"
      final dt = DateTime.tryParse(s.replaceAll(' PHT', ''));
      if (dt == null) return s;
...
void _showReportDialog(BuildContext context) {
  ...
  final latController = TextEditingController(text: '14.5995');
  final lonController = TextEditingController(text: '120.9842');
  String? selectedType;
            ...
            children: [
              ...
              DropdownButton<String>(
                ...
                items: const [
                  ...
                  DropdownMenuItem(value: 'heavy_traffic', child: Text('Heavy Traffic')),
                  DropdownMenuItem(value: 'accident', child: Text('Accident')),
                  DropdownMenuItem(value: 'road_damage', child: Text('Road Damage')),
                  DropdownMenuItem(value: 'dark_area', child: Text('Dark/Unlit Area')),
                  DropdownMenuItem(value: 'safe_spot', child: Text('Safe Spot')),
                ]
              ...
              const SizedBox(height: 12),
              // Location fields
              Text('Location (lat, lon)', style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w600, fontSize: 13)),
              const SizedBox(height: 8),
              Row(children: [
            ...
            onPressed: isSubmitting
                ...
                : () async {
                    ...
                    try {
                      ...
                      final lat = double.tryParse(latController.text) ?? 14.5995;
                      final lon = double.tryParse(lonController.text) ?? 120.9842;
